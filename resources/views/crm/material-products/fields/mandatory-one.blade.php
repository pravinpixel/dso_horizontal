            ...
            <div class="col-8">
                @if (wizard_mode() == 'create' || session()->get('edit_mode') == 'parent')
                    ...
                    <select required onchange="change_product_type()" class="form-select" id="category_type" {{ config(is_disable(category_type() ?? $material_product->category_selection ?? null)."category_selection.status") }}>
                        ...
                        <option {{ category_type() ==  'material' ? 'selected' : null }} {{ $material_product->category_selection ==  'material' ? 'selected' : null }} value="material">Material</option>
                        <option {{ category_type() ==  'in_house' ? 'selected' : null }} {{ $material_product->category_selection ==  'in_house' ? 'selected' : null }} value="in_house">In-House Products</option> 
                    </select>
                        @else
